<div class="mx-auto p-6 rounded-lg text-white bg-[#222222]">
    <form action="{{ route('create-comment', ['movie' => $movie->id]) }}" method="POST">
        ...
        <div x-data="{ rating: 0 }" class="flex justify-center gap-1 text-yellow-400 mb-4">
            @for ($i = 1; $i <= 5; $i++)
                <img src="{{ asset('assets/star-gray.png') }}" alt="Star"
                    class="w-8 h-8 cursor-pointer transition-transform transform hover:scale-110"
                    :src="rating >= {{ $i }} ? '{{ asset('assets/star.png') }}' :
                        '{{ asset('assets/star-gray.png') }}'"
                    @click="rating = {{ $i }}">
            @endfor
        </div>
        <!-- Review Editor -->
        ...
        <input type="hidden" name='parent_id' value="{{ $parentComment->id ?? null }}">
